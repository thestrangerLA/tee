...
const calculatorStateDocRef = doc(db, 'appliance-cashCalculatorState', 'latest');
...
export const listenToApplianceCashCalculatorState = (callback: (state: CashCalculatorState) => void) => {
    ensureInitialState(); 
...
export const updateApplianceCashCalculatorState = async (newState: Partial<Omit<CashCalculatorState, 'id'>>) => {
