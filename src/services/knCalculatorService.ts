...
const calculationsCollectionRef = collection(db, 'knCalculations');
...
export const addCalculationToHistory = async (calculationId: string, snapshot: Omit<CalculationSnapshot, 'id'|'savedAt'>): Promise<string> => {
    const docRef = doc(calculationsCollectionRef, calculationId);
    const newSnapshot = {
        ...convertDatesToTimestamps(snapshot),
        id: new Date().toISOString(), // Simple unique ID for the snapshot
        savedAt: serverTimestamp()
    }
    
    await updateDoc(docRef, {
        history: arrayUnion(newSnapshot)
    });
...
export const deleteCalculationSnapshot = async (calculationId: string, snapshotId: string): Promise<void> => {
    ...
    if (!calculation || !calculation.history) return;

    const snapshotToDelete = calculation.history.find(s => s.id === snapshotId);
    if (snapshotToDelete) {
        await updateDoc(docRef, {
            history: arrayRemove(snapshotToDelete)
        });
    }
}
