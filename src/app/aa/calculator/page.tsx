...
import { ArrowLeft, Calculator, PlusCircle, MoreHorizontal, ChevronDown, Calendar as CalendarIcon, Filter } from "lucide-react";
import { deleteCalculation, listenToSavedCalculations } from '@/services/aaCalculatorService';
...
export default function AACalculationsListPage() {
    ...
    const handleDeleteCalculation = async (calculationId: string, groupCode: string) => {
        ...
        try {
            ...
            toast({
                title: "ລົບການຄຳນວນສຳເລັດ",
            });
        } catch (error) {
            console.error("Error deleting calculation:", error);
            toast({
                title: "ເກີດຂໍ້ຜິດພາດ",
                description: "ບໍ່ສາມາດລົບການຄຳນວນໄດ້",
                variant: "destructive",
            });
        }
    };
        ...
        <div className="flex min-h-screen w-full flex-col bg-muted/40">
            <header className="sticky top-0 z-30 flex h-14 items-center gap-4 border-b bg-background px-4 sm:static sm:h-auto sm:border-0 sm:bg-transparent sm:px-6">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <h1 className="text-xl font-bold tracking-tight font-headline">ລາຍການຄຳນວນທັງໝົດ (AA)</h1>
                </div>
                 <div className="ml-auto flex items-center gap-4">
                    <YearSelector />
                    <Link href="/aa/calculator/new">
                        <Button size="sm">
            ...
            <main className="flex flex-1 flex-col gap-4 p-4 sm:px-6 sm:py-0 md:gap-8">
                <Card>
                    <CardHeader>
                        <CardTitle>ລາຍການຄຳນວນ {selectedYear ? `ປີ ${selectedYear + 543}` : 'ທັງໝົດ'}</CardTitle>
                        <CardDescription>
                            ຈັດການ, ສ້າງ ແລະ ແກ້ໄຂລາຍການຄຳນວນຄ່າໃຊ້ຈ່າຍ
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
