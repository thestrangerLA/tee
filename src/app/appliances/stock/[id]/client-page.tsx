...
import { listenToApplianceStockLogs, updateApplianceStockItem, deleteApplianceStockLog, updateApplianceStockLog } from '@/services/applianceStockService';
...
export default function ApplianceStockClientPage({ initialItem }: { initialItem: ApplianceStockItem }) {
    const { toast } = useToast();
    const id = initialItem.id;

    const [item, setItem] = useState<ApplianceStockItem | null>(initialItem);
    const [logs, setLogs] = useState<ApplianceStockLog[]>([]);
    const [isEditing, setIsEditing] = useState(false);
    const [editData, setEditData] = useState<Partial<ApplianceStockItem>>(initialItem);
    
    const [editingLog, setEditingLog] = useState<ApplianceStockLog | null>(null);

    useEffect(() => {
        if (!id) return;
        const unsubscribeLogs = listenToApplianceStockLogs(id, setLogs);
    ...
    const handleSave = async () => {
        if (!id) return;
        try {
            await updateApplianceStockItem(id, editData);
     ...
     const handleDeleteLog = async (logId: string, itemId: string) => {
        ...
        try {
            await deleteApplianceStockLog(logId, itemId);
    ...
    const handleUpdateLog = async () => {
        ...
        try {
            await updateApplianceStockLog(editingLog.id, editingLog.itemId, {
    ...
    return (
        <div className="flex min-h-screen w-full flex-col bg-muted/40">
            ...
            <main className="flex flex-1 flex-col gap-4 p-4 sm:px-6 sm:py-0 md:gap-8">
                <Card>
                    <CardHeader>
                        <CardTitle>ລາຍລະອຽດສິນຄ້າ (ເຄື່ອງໃຊ້)</CardTitle>
                    </CardHeader>
