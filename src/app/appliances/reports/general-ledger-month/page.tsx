...
function ApplianceGeneralLedgerMonthPageComponent() {
    ...
    useEffect(() => {
        ...
        setError(null);
        try {
            const unsubscribe = listenToApplianceTransactions(
        ...
        <div className="flex min-h-screen w-full flex-col bg-muted/40 print:bg-white">
            <header className="sticky top-0 z-30 flex h-14 items-center gap-4 border-b bg-background px-4 sm:static sm:h-auto sm:border-0 sm:bg-transparent sm:px-6 print:hidden">
                <Button variant="outline" size="icon" className="h-8 w-8" asChild>
                    <Link href="/appliances/reports/general-ledger">
            ...
            <main className="flex flex-1 flex-col gap-4 p-4 sm:px-6 sm:py-0 md:gap-8 print:p-2 print:gap-2">
                <div className="hidden print:block text-center mb-4">
                    <h1 className="text-xl font-bold">ປະຫວັດຮັບ-ຈ່າຍທົ່ວໄປ (ທຸລະກິດເຄື່ອງໃຊ້)</h1>
...
export default function ApplianceGeneralLedgerMonthPageWrapper() {
    return (
        <StaticExportWrapper>
            <ApplianceGeneralLedgerMonthPageComponent />
        </StaticExportWrapper>
    )
}
