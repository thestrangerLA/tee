...
import { listenToSpsMeatAccountSummary, updateSpsMeatAccountSummary, listenToSpsMeatTransactions, addSpsMeatTransaction, updateSpsMeatTransaction, deleteSpsMeatTransaction } from '@/services/spsMeatAccountancyService';
import { listenToSpsMeatCalculatorState, updateSpsMeatCalculatorState } from '@/services/spsMeatCashCalculatorService';
...
const CashCalculatorCard = ({ onTotalChange }: { onTotalChange: (total: number) => void }) => {
    ...
    useEffect(() => {
        const unsubscribe = listenToSpsMeatCalculatorState(setCalculatorState);
        return () => unsubscribe();
    }, []);
    ...
    const handleCountChange = async (key: string, value: string) => {
        ...
        await updateSpsMeatCalculatorState({ counts: newCounts }); 
    };

    const handleReset = async () => {
        await updateSpsMeatCalculatorState({ counts: initialCounts });
    };
...
export default function SpsMeatAccountancyPage() {
    ...
    useEffect(() => {
        const unsubscribeSummary = listenToSpsMeatAccountSummary(setSummary);
        const unsubscribeTransactions = listenToSpsMeatTransactions(setTransactions);
        return () => {
            unsubscribeSummary();
    ...
    const handleCalculatorTotalChange = async (totalKip: number) => {
        if (summary && totalKip !== summary.cash) {
            await updateSpsMeatAccountSummary({ cash: totalKip });
        }
    };
    ...
    const handleUpdateTransaction = async () => {
        ...
        try {
            await updateSpsMeatTransaction(editingTransaction.id, {
                ...editingTransaction,
    ...
    const handleDeleteTransaction = async (id: string) => {
        ...
        try {
            await deleteSpsMeatTransaction(id);
    ...
    const handleSaveSummary = async () => {
        ...
        try {
            await updateSpsMeatAccountSummary({ [editingField]: editingValue });
        ...
        <div className="flex min-h-screen w-full flex-col bg-muted/40">
            <header className="sticky top-0 z-30 flex h-14 items-center gap-4 border-b bg-background px-4 sm:static sm:h-auto sm:border-0 sm:bg-transparent sm:px-6">
                <Button variant="outline" size="icon" className="h-8 w-8" asChild>
                    <Link href="/meat-business/sps-meat"><ArrowLeft className="h-4 w-4" /></Link>
                </Button>
                <h1 className="text-xl font-bold tracking-tight">ຈັດການບັນຊີ (SPS Meat)</h1>
            </header>
