...
export default function SpsMeatGeneralLedgerPage() {
    ...
    useEffect(() => {
        const unsubscribeTransactions = listenToAllSpsMeatTransactions(setTransactions);
        return () => {
        ...
        <div className="flex min-h-screen w-full flex-col bg-muted/40">
            <header className="sticky top-0 z-30 flex h-14 items-center gap-4 border-b bg-background px-4 sm:static sm:h-auto sm:border-0 sm:bg-transparent sm:px-6 print:hidden">
                <Button variant="outline" size="icon" className="h-8 w-8" asChild>
                    <Link href="/meat-business/sps-meat/reports">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <h1 className="text-xl font-bold tracking-tight">ປະຫວັດຮັບ-ຈ່າຍ (SPS Meat)</h1>
                </div>
            </header>
                        ...
                        {reportData.monthlyReports.length > 0 ? (
                           <div className="space-y-2">
                            {reportData.monthlyReports.map(({ year, month, net }) => (
                                <Link 
                                    href={`/meat-business/sps-meat/reports/general-ledger-month?year=${year}&month=${month}`} 
                                    key={`${year}-${month}`}
                                    passHref
                                >
