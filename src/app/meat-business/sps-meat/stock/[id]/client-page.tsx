...
import { listenToSpsMeatStockLogs, updateSpsMeatStockItem, deleteSpsMeatStockLog, updateSpsMeatStockLog } from '@/services/spsMeatStockService';
...
export default function SpsMeatStockClientPage({ initialItem }: { initialItem: MeatStockItem }) {
    ...
    useEffect(() => {
        if (!id) return;
        const unsubscribeLogs = listenToSpsMeatStockLogs(id, setLogs);
    ...
    const handleSave = async () => {
        if (!id) return;
        try {
            await updateSpsMeatStockItem(id, editData);
     ...
     const handleDeleteLog = async (logId: string, itemId: string) => {
        ...
        try {
            await deleteSpsMeatStockLog(logId, itemId);
    ...
    const handleUpdateLog = async () => {
        ...
        try {
            await updateSpsMeatStockLog(editingLog.id, editingLog.itemId, {
        ...
        <div className="flex min-h-screen w-full flex-col bg-muted/40">
            <header className="sticky top-0 z-30 flex h-14 items-center gap-4 border-b bg-background px-4 sm:static sm:h-auto sm:border-0 sm:bg-transparent sm:px-6">
                <Button variant="outline" size="icon" className="h-8 w-8" asChild>
                    <Link href="/meat-business/sps-meat/stock">
            ...
            <main className="flex flex-1 flex-col gap-4 p-4 sm:px-6 sm:py-0 md:gap-8">
                <Card>
                    <CardHeader>
                        <CardTitle>ລາຍລະອຽດສິນຄ້າ (SPS Meat)</CardTitle>
                    </CardHeader>
