...
import { listenToAllMeatTransactions } from '@/services/meatReportService';
...
export default function MeatGeneralLedgerPage() {
    ...
    useEffect(() => {
        const unsubscribeTransactions = listenToAllMeatTransactions(setTransactions);
        return () => {
        ...
        <div className="flex min-h-screen w-full flex-col bg-muted/40">
            <header className="sticky top-0 z-30 flex h-14 items-center gap-4 border-b bg-background px-4 sm:static sm:h-auto sm:border-0 sm:bg-transparent sm:px-6 print:hidden">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <h1 className="text-xl font-bold tracking-tight">ປະຫວັດຮັບ-ຈ່າຍທົ່ວໄປ (ທຸລະກິດຊີ້ນ)</h1>
                </div>
            </header>
                        ...
                        {reportData.monthlyReports.length > 0 ? (
                           <div className="space-y-2">
                            {reportData.monthlyReports.map(({ year, month, net }) => (
                                <Link 
                                    href={`/meat-business/reports/general-ledger-month?year=${year}&month=${month}`} 
                                    key={`${year}-${month}`}
                                    passHref
                                >
